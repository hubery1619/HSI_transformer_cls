...
def proposed(dataset, patch_size, trans_type):
    ...
    if dataset == 'sa':
        ...
        model = HyperTransformer(img_size=patch_size, block_type = trans_type, in_chans=144, num_classes=15, n_groups=[1, 1, 1, 1], depths=[3, 3, 2, 2], embed_dims=[128, 64, 32, 16], num_heads_spatial=[2, 2, 2, 2])
    elif dataset == 'indian':
        ...
        model = HyperTransformer(img_size=patch_size, block_type = trans_type, in_chans=145, num_classes=14, n_groups=[1, 1, 1, 1], depths=[3, 3, 2, 2], embed_dims=[96, 64, 32, 32], num_heads_spatial=[2, 2, 2, 2])
    elif dataset == 'pu':
        model = HyperTransformer(img_size=patch_size, block_type = trans_type, in_chans=103, num_classes=9, n_groups=[1, 1, 1, 1], depths=[2, 2, 6, 2], embed_dims=[96, 64, 32, 16], num_heads_spatial=[8, 8, 8, 8])
    elif dataset == 'ksc':
...
if __name__ == "__main__":
    ...
    # net = proposed(dataset='sa', patch_size=7, block_type=0)
    # print("output shape:", net(t).shape)

    t = torch.randn(size=(64, 1, 103, 11, 11))
    print("input shape:", t.shape)
    net = proposed(dataset='pu', patch_size=11, trans_type=2)
    print("output shape:", net(t).shape)
    with torch.no_grad():
        summary(net, t)
    flops = FlopCountAnalysis(net, t)
    mflops = (flops.total())/1e6
    print(f"FLOPs: {mflops:.2f}")